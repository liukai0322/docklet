...
def parse_cookie(cookie, securekey):
    ...
    try:
        ...
        print ("decode cookie failed")
        return None
    print ("cookie content : %s" % text)
    thatpart2 = hashlib.md5((text+securekey).encode('ascii')).hexdigest()
    print ("hash from part1 : %s" % thatpart2)
    print ("hash from part2 : %s" % part2)
    if part2 == thatpart2:
        result = json.loads(text)['name']
    else:
        result = None
    print ("parse from cookie : %s" % result)
    return result
